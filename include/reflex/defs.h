...
namespace Reflex
{
	...
	enum KeyCode
	{
		...
		// from SDK scan codes
		KEY_NONUSHASH = NATIVE_VK(-54,  -34,  0x32),
		KEY_POWER     = NATIVE_VK(-55,  -35,  0x66),
		KEY_MENU      = NATIVE_VK(-56,  -36,  0x76),

		KEY_STOP  = NATIVE_VK(-57,  -37,  0x78),
		KEY_AGAIN = NATIVE_VK(-58,  -38,  0x79),
		KEY_UNDO  = NATIVE_VK(-59,  -39,  0x7A),
		KEY_CUT   = NATIVE_VK(-60,  -40,  0x7B),
		KEY_COPY  = NATIVE_VK(-61,  -41,  0x7C),
		KEY_PASTE = NATIVE_VK(-62,  -42,  0x7D),
		KEY_FIND  = NATIVE_VK(-63,  -43,  0x7E),

		KEY_NUM_EQUALSAS400 = NATIVE_VK(-64,  -44,  0x86),

		KEY_INTERNATIONAL2 = NATIVE_VK(-65,  -45,  0x88),
		KEY_INTERNATIONAL4 = NATIVE_VK(-66,  -46,  0x8A),
		KEY_INTERNATIONAL5 = NATIVE_VK(-67,  -47,  0x8B),
		KEY_INTERNATIONAL6 = NATIVE_VK(-68,  -48,  0x8C),
		KEY_INTERNATIONAL7 = NATIVE_VK(-69,  -49,  0x8D),
		KEY_INTERNATIONAL8 = NATIVE_VK(-70,  -50,  0x8E),
		KEY_INTERNATIONAL9 = NATIVE_VK(-71,  -51,  0x8F),

		KEY_LANG1 = NATIVE_VK(-72,  -52,  0x90),// Hangul/English toggle
		KEY_LANG2 = NATIVE_VK(-73,  -53,  0x91),// Hanja conversion
		KEY_LANG3 = NATIVE_VK(-74,  -54,  0x92),// Katakana
		KEY_LANG4 = NATIVE_VK(-75,  -55,  0x93),// Hiragana
		KEY_LANG5 = NATIVE_VK(-76,  -56,  0x94),// Zenkaku/Hankaku
		KEY_LANG6 = NATIVE_VK(-77,  -57,  0x95),
		KEY_LANG7 = NATIVE_VK(-78,  -58,  0x96),
		KEY_LANG8 = NATIVE_VK(-79,  -59,  0x97),
		KEY_LANG9 = NATIVE_VK(-80,  -60,  0x98),

		KEY_ALTERASE   = NATIVE_VK(-81,  -61,  0x99),
		KEY_SYSREQ     = NATIVE_VK(-82,  -62,  0x9A),
		KEY_CANCEL     = NATIVE_VK(-83,  -63,  0x9B),
		KEY_PRIOR      = NATIVE_VK(-84,  -64,  0x9D),
		KEY_RETURN2    = NATIVE_VK(-85,  -65,  0x9E),
		KEY_SEPARATOR  = NATIVE_VK(-86,  -66,  0x9F),
		KEY_OUT        = NATIVE_VK(-87,  -67,  0xA0),
		KEY_OPER       = NATIVE_VK(-88,  -68,  0xA1),
		KEY_CLEARAGAIN = NATIVE_VK(-89,  -69,  0xA2),
		KEY_CRSEL      = NATIVE_VK(-90,  -70,  0xA3),
		KEY_EXSEL      = NATIVE_VK(-91,  -71,  0xA4),

		KEY_NUM_00             = NATIVE_VK(-92,  -72,  0xB0),
		KEY_NUM_000            = NATIVE_VK(-93,  -73,  0xB1),
		KEY_THOUSANDSSEP       = NATIVE_VK(-94,  -74,  0xB2),
		KEY_DECIMALSEP         = NATIVE_VK(-95,  -75,  0xB3),
		KEY_CURRENCYUNIT       = NATIVE_VK(-96,  -76,  0xB4),
		KEY_CURRENCYSUBUNIT    = NATIVE_VK(-97,  -77,  0xB5),
		KEY_NUM_LPAREN         = NATIVE_VK(-98,  -78,  0xB6),
		KEY_NUM_RPAREN         = NATIVE_VK(-99,  -79,  0xB7),
		KEY_NUM_LBRACE         = NATIVE_VK(-100, -80,  0xB8),
		KEY_NUM_RBRACE         = NATIVE_VK(-101, -81,  0xB9),
		KEY_NUM_TAB            = NATIVE_VK(-102, -82,  0xBA),
		KEY_NUM_BACKSPACE      = NATIVE_VK(-103, -83,  0xBB),
		KEY_NUM_A              = NATIVE_VK(-104, -84,  0xBC),
		KEY_NUM_B              = NATIVE_VK(-105, -85,  0xBD),
		KEY_NUM_C              = NATIVE_VK(-106, -86,  0xBE),
		KEY_NUM_D              = NATIVE_VK(-107, -87,  0xBF),
		KEY_NUM_E              = NATIVE_VK(-108, -88,  0xC0),
		KEY_NUM_F              = NATIVE_VK(-109, -89,  0xC1),
		KEY_NUM_XOR            = NATIVE_VK(-110, -90,  0xC2),
		KEY_NUM_POWER          = NATIVE_VK(-111, -91,  0xC3),
		KEY_NUM_PERCENT        = NATIVE_VK(-112, -92,  0xC4),
		KEY_NUM_LESS           = NATIVE_VK(-113, -93,  0xC5),
		KEY_NUM_GREATER        = NATIVE_VK(-114, -94,  0xC6),
		KEY_NUM_AMPERSAND      = NATIVE_VK(-115, -95,  0xC7),
		KEY_NUM_DBLAMPERSAND   = NATIVE_VK(-116, -96,  0xC8),
		KEY_NUM_VERTICALBAR    = NATIVE_VK(-117, -97,  0xC9),
		KEY_NUM_DBLVERTICALBAR = NATIVE_VK(-118, -98,  0xCA),
		KEY_NUM_COLON          = NATIVE_VK(-119, -99,  0xCB),
		KEY_NUM_HASH           = NATIVE_VK(-120, -100, 0xCC),
		KEY_NUM_SPACE          = NATIVE_VK(-121, -101, 0xCD),
		KEY_NUM_AT             = NATIVE_VK(-122, -102, 0xCE),
		KEY_NUM_EXCLAM         = NATIVE_VK(-123, -103, 0xCF),
		KEY_NUM_MEMSTORE       = NATIVE_VK(-124, -104, 0xD0),
		KEY_NUM_MEMRECALL      = NATIVE_VK(-125, -105, 0xD1),
		KEY_NUM_MEMCLEAR       = NATIVE_VK(-126, -106, 0xD2),
		KEY_NUM_MEMADD         = NATIVE_VK(-127, -107, 0xD3),
		KEY_NUM_MEMSUBTRACT    = NATIVE_VK(-128, -108, 0xD4),
		KEY_NUM_MEMMULTIPLY    = NATIVE_VK(-129, -109, 0xD5),
		KEY_NUM_MEMDIVIDE      = NATIVE_VK(-130, -110, 0xD6),
		KEY_NUM_PLUSMINUS      = NATIVE_VK(-131, -111, 0xD7),
		KEY_NUM_CLEARENTRY     = NATIVE_VK(-132, -112, 0xD9),
		KEY_NUM_BINARY         = NATIVE_VK(-133, -113, 0xDA),
		KEY_NUM_OCTAL          = NATIVE_VK(-134, -114, 0xDB),
		KEY_NUM_HEXADECIMAL    = NATIVE_VK(-135, -115, 0xDD),

		KEY_MODE = NATIVE_VK(-136, -116, 0x101),

		KEY_AUDIOMUTE  = NATIVE_VK(-137, -117, 0x106),
		KEY_WWW        = NATIVE_VK(-138, -118, 0x108),
		KEY_CALCULATOR = NATIVE_VK(-139, -119, 0x10A),
		KEY_COMPUTER   = NATIVE_VK(-140, -120, 0x10B),

		KEY_BRIGHTNESSDOWN = NATIVE_VK(-141, -121, 0x113),
		KEY_BRIGHTNESSUP   = NATIVE_VK(-142, -122, 0x114),
		KEY_DISPLAYSWITCH  = NATIVE_VK(-143, -123, 0x115),
		KEY_KBDILLUMTOGGLE = NATIVE_VK(-144, -124, 0x116),
		KEY_KBDILLUMDOWN   = NATIVE_VK(-145, -125, 0x117),
		KEY_KBDILLUMUP     = NATIVE_VK(-146, -126, 0x118),
		KEY_EJECT          = NATIVE_VK(-147, -127, 0x119),

		KEY_AUDIOREWIND      = NATIVE_VK(-148, -128, 0x11D),
		KEY_AUDIOFASTFORWARD = NATIVE_VK(-149, -129, 0x11E),

		KEY_SOFTLEFT  = NATIVE_VK(-150, -130, 0x11F),
		KEY_SOFTRIGHT = NATIVE_VK(-151, -131, 0x120),
		KEY_CALL      = NATIVE_VK(-152, -132, 0x121),
		KEY_ENDCALL   = NATIVE_VK(-153, -133, 0x122),
