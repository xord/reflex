...
module Reflex
  ...
  class PointerEvent < Event
    ...
    def_delegators :first,
      ...
      :position, :pos, :x, :y, :modifiers, :drag?, :click_count, :layer,
      :time, :prev
    ...
    def inspect()
      "#<Reflex::PointerEvent id:#{id} #{types} #{action} (#{x.round 2}, #{y.round 2}) mod:#{modifiers} drag:#{drag?} click:#{click_count} layer:#{layer} time:#{time.round 2}>"
    end
