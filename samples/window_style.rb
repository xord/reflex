...
# How a window can look: which parts of the titlebar it shows, whether it
# draws a shadow, and whether the alpha you draw with lets the desktop show
# through.
#
...
#   4: shadow, 5: transparent, ESC: quit
class StyledWindow < Reflex::Window
  ...
  def on_draw (e)
    ...
    p.text "shadow: #{shadow?}", 10, 30
    p.text "transparent: #{transparent?}", 10, 50
    p.text "#{e.fps.to_i} FPS", 10, 70
    p.text '1: buttons, 2: background, 3: both', 10, 90
    p.text '4: shadow, 5: transparent, ESC: quit', 10, 110

    if @@message
      p.fill 1, 0.4, 0.4
      p.text @@message, 10, 130
    end
  end
